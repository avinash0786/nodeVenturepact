...
sgMail.setApiKey(
  "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"
);
...
exports.saveFilePost = function (req, res) {
  ...
  res.send(
    "File saved successfully, to download hit http://localhost:3000/getFile/" +
...
exports.listUploadedFiles = function (req, res) {
  ...
  fs.readdir(
    path.join(__dirname, "../temp/fileUploads"),
    function (err, files) {
      if (err) {
...
exports.savePdfData = function (req, res) {
  ...
  var html = fs.readFileSync(
    path.join(__dirname, "../views/pdfOutLayout.html"),
    "utf8"
  );
  ...
  var document = {
    ...
    path: path.join(
      __dirname,
      `../temp/pdfFiles/${req.body.fname} ${req.body.lname}.pdf`
    ),
    type: "",
  };
